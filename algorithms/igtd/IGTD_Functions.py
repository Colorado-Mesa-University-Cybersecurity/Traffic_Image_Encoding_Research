# Note, this is the one file we didn't write. We did make edits though. It is available at https://github.com/zhuyitan/IGTD 
...
def generate_image_data(data, index, num_row, num_column, coord, image_folder=None, file_name=''):
    ...
    for i in range(data_2.shape[0]):
        ...
        if image_folder is not None:
            new_p = Image.fromarray(data_i)
            new_p = new_p.convert("L")
            new_p.save(image_folder + '/' + file_name + '_' + samples[i] + '_image.png')
            ...
            #            pad_inches=0)
            plt.imsave(fname=image_folder + '/' + file_name + '_' + samples[i] + '_image.png', arr=data_i, cmap='gray_r', format='png')
            plt.close(fig) """
...
def table_to_image(norm_d, scale, fea_dist_method, image_dist_method, save_image_size, max_step, val_step, normDir,
    ...
    cp.dump(time, output)
    output.close() '''
    return scale,coordinate,normDir,min_id,index
